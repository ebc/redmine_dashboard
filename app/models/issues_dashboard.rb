class IssuesDashboard
  ...
  def self.issues_completed_count
    sql = "SELECT COUNT(id) completed FROM #{Issue.table_name} WHERE done_ratio = 100"
    result = ActiveRecord::Base.connection.select_one(sql)
    result['completed'].to_f
  end

  # Calculate done ratio from all issues
  def self.issues_done_ratio
    sql = "SELECT SUM(done_ratio) done, COUNT(id) total FROM #{Issue.table_name}"
